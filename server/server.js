process.on('unhandledRejection', (reason, promise) => {
  ...
  // Здесь можно завершить процесс или попробовать перезапустить его через менеджер процессов
  process.exit(1); // Завершаем процесс (это безопаснее, чем продолжать работать в полусломанном состоянии)
});
...
const { port, host, dbUri, rateLimit: rateLimitConfig, 
  logging, compression: compressionConfig } = require('./config');
...
// 'combined' - стандартный формат логов Apache
...
const limiter = rateLimit({
  windowMs: rateLimitConfig.windowMs, // (15) минут
  max: rateLimitConfig.max, // максимум (100) запросов с одного IP за windowMs
  standardHeaders: true, // добавляет стандартные заголовки rate-limit к ответам
...
app.use('/images', (req, res, next) => {
  ...
  res.setHeader('Cross-Origin-Resource-Policy', 'cross-origin'); // важно
  next();
...
// Проксирование запросов к Vite dev серверу в режиме разработки
if (process.env.NODE_ENV !== 'production') {
